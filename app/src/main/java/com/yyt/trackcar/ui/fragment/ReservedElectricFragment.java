 ...
 * @ fileName:      ReservedElectricFragment
 * @ author:        QING
 * @ createTime:    2020/3/13 16:04
 * @ describe:      TODO 预留电量
 */
@Page(name = "ReservedElectric")
public class ReservedElectricFragment extends BaseFragment {
    ...
    protected TitleBar initTitle() {
        ...
        titleBar.setTitle(R.string.reserved_electric);
        return titleBar;
    }
    ...
    protected void initViews() {
        ...
        if (settingsModel != null && !TextUtils.isEmpty(settingsModel.getOther())) {
            ...
            if (array.length >= 5) {
                String[] subArray = array[4].split("\\|");
                mIsOpen = subArray.length >= 2 && "1".equals(subArray[1]);
            }
        }
        setSwitch();
        getOther();
    ...
    private void initItems() {
        BaseItemBean itemBean = new BaseItemBean(getString(R.string.watch_ban),
                getString(R.string.watch_ban_content));
        itemBean.setBgDrawable(android.R.color.transparent);
        mItemList.add(new SectionItem(itemBean));
        itemBean = new BaseItemBean(getString(R.string.support_answer_call),
                getString(R.string.support_answer_call_content));
        itemBean.setBgDrawable(android.R.color.transparent);
        mItemList.add(new SectionItem(itemBean));
    ...
    private void initHeaderView() {
        ...
        tvTitle.setText(R.string.reserved_electric_title);
        if (SettingSPUtils.getInstance().getInt(CWConstant.DEVICE_TYPE, 0) == 0) {
            tvContent.setText(R.string.reserved_electric_content);
            ivBg.setImageResource(R.mipmap.bg_reserved_electric);
        } else {
            tvContent.setText(R.string.reserved_electric_device_content);
            ivBg.setImageResource(R.mipmap.bg_reserved_electric_second);
        }
    ...
    private void setOther() {
        ...
        if (userModel != null && deviceModel != null) {
            ...
            if (!TextUtils.isEmpty(settingsModel.getOther())) {
                ...
                if (array.length >= 5) {
                    ...
                    if (subArray.length >= 2)
                        array[4] = String.format("%s|%s", subArray[0], mIsOpen ? "1" : "0");
                }
            ...
            if (TextUtils.isEmpty(other))
                other = String.format("5,0,0,06:00|22:00|1,20|%s", mIsOpen ? "1" : "0");
            else
                                ...
                                if (userModel != null && deviceModel != null && deviceModel.getD_id() == requestBean.getD_id()) {
                                    ...
                                    if (!TextUtils.isEmpty(bean.getOther())) {
                                        ...
                                        if (array.length >= 5) {
                                            String[] subArray = array[4].split("\\|");
                                            mIsOpen =
                                                    subArray.length >= 2 && "1".equals(subArray[1]);
                                        } else
                                            mIsOpen = false;
                                    } else
                                        mIsOpen = false;
