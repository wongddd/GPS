...
public class PermissionUtils {
    ...
    private static final int REQUEST_PERMISSION_BLE = 9360;
    ...
    public static void checkMorePermissions(Context context, String[] permissions,
        ...
        if (permissionList.size() == 0) {  // 用户已授予权限
            ...
            String[] unauthorizedMorePermissions =
...
//        }
    ...
    public static void requestScanPermission(final Activity activity, final RxBleClient client) {
        ...
        ActivityCompat.requestPermissions(
                activity,
                /*
                 * the below would cause a ArrayIndexOutOfBoundsException on API < 23. Yet it
                 * should not be called then as runtime
                 * permissions are not needed and RxBleClient.isScanRuntimePermissionGranted()
                 * returns `true`
                 */
    ...
    public static boolean isScanPermissionGranted(final int requestCode, final String[] permissions,
        ...
        if (requestCode != REQUEST_PERMISSION_BLE_SCAN || client == null) {
            return false;
        }
    ...
    public static void requestConnectionPermission(final Activity activity,
        ...
        ActivityCompat.requestPermissions(
                activity,
                /*
                 * the below would cause a ArrayIndexOutOfBoundsException on API < 31. Yet it
                 * should not be called then as runtime
                 * permissions are not needed and RxBleClient.isConnectRuntimePermissionGranted()
                 *  returns `true`
                 */
    ...
    public static boolean isRequestConnectionPermissionGranted(final int requestCode,
                                                               final String[] permissions,
                                                               final int[] grantResults,
                                                               RxBleClient client) {
        if (requestCode != REQUEST_PERMISSION_BLE_CONNECT || client == null) {
            return false;
        }

        String[] recommendedConnectRuntimePermissions =
    ...
    public static void requestBlePermission(final Activity activity,
        ...
        ActivityCompat.requestPermissions(activity,
                new String[]{client.getRecommendedScanRuntimePermissions()[0],
                        client.getRecommendedConnectRuntimePermissions()[0]},
                REQUEST_PERMISSION_BLE_CONNECT
        );
    }

    public static boolean isRequestBlePermissionGranted(final int requestCode,
                                                               final String[] permissions,
                                                               final int[] grantResults,
                                                               RxBleClient client) {
        if (requestCode != REQUEST_PERMISSION_BLE_CONNECT || client == null) {
        ...
        String[] recommendedConnectRuntimePermissions =
                client.getRecommendedConnectRuntimePermissions();
        String[] recommendedScanRuntimePermissions = client.getRecommendedScanRuntimePermissions();
        boolean connectPermissions = false;
        boolean scanPermissions = false;
        for (int i = 0; i < permissions.length; i++) {
            for (String recommendedConnectRuntimePermission : recommendedConnectRuntimePermissions) {
                if (permissions[i].equals(recommendedConnectRuntimePermission)
                        && grantResults[i] == PackageManager.PERMISSION_GRANTED) {
                    connectPermissions = true;
                    break;
                }
            }
            for (String recommendedScanRuntimePermission : recommendedScanRuntimePermissions) {
            ...
            if(connectPermissions && scanPermissions){
                return true;
            }
        }
